"use client";

import { signIn } from "next-auth/react";
import { Train } from "lucide-react";
import { Suspense } from "react";
import { useSearchParams } from "next/navigation";

const DOMAINS = ["mayoreo.biz", "febeca.com", "mundipartes.com", "cofersa.cr", "sillaca.biz", "beval.biz"];

function LoginInner() {
  const params = useSearchParams();
  const error = params.get("error");

  return (
    <div className="min-h-screen grid place-items-center px-4" style={{ background: "#F6F8FB" }}>
      <div className="w-full max-w-sm rounded-2xl p-8 text-center" style={{ background: "#FFFFFF", border: "1px solid #E2E7EF", boxShadow: "0 20px 60px rgba(16,24,40,.12)" }}>
        <div className="grid place-items-center w-14 h-14 rounded-2xl mx-auto mb-4" style={{ background: "linear-gradient(135deg,#059669,#2563EB)" }}>
          <Train size={26} className="text-white" />
        </div>
        <div className="text-[11px] uppercase tracking-[.22em]" style={{ color: "#8C95A6" }}>Centro de Control · COFERSA</div>
        <h1 className="text-xl font-bold tracking-tight mt-1 mb-1" style={{ color: "#1A1F2B" }}>Metro de Proyectos</h1>
        <p className="text-sm mb-6" style={{ color: "#5A6577" }}>Ingresa con tu cuenta corporativa de Google.</p>

        {error && (
          <div className="text-xs rounded-lg px-3 py-2 mb-4 text-left" style={{ background: "#FEF2F2", color: "#DC2626", border: "1px solid #FCA5A5" }}>
            {error === "Configuration" ? (
              <span>
                <strong>Error de configuración:</strong> Faltan credenciales de Google OAuth (<code>GOOGLE_CLIENT_ID</code> / <code>GOOGLE_CLIENT_SECRET</code>) o <code>NEXTAUTH_SECRET</code> en las variables de entorno de tu servidor o archivo <code>.env</code>.
              </span>
            ) : error === "AccessDenied" ? (
              <span>
                Tu cuenta no tiene acceso. Usa un correo de un dominio autorizado de la organización (ej: <code>mayoreo.biz</code>).
              </span>
            ) : (
              <span>
                Error al iniciar sesión ({error}). Por favor, verifica tus credenciales o intenta de nuevo.
              </span>
            )}
          </div>
        )}

        <button
          onClick={() => signIn("google", { callbackUrl: "/" })}
          className="w-full flex items-center justify-center gap-2.5 rounded-lg px-4 py-2.5 text-sm font-semibold transition hover:-translate-y-px"
          style={{ background: "#FFFFFF", border: "1px solid #D3DAE6", color: "#1A1F2B" }}
        >
          <svg width="18" height="18" viewBox="0 0 48 48" aria-hidden="true">
            <path fill="#FFC107" d="M43.6 20.5H42V20H24v8h11.3C33.7 32.4 29.3 35 24 35c-6.1 0-11-4.9-11-11s4.9-11 11-11c2.8 0 5.4 1.1 7.3 2.8l5.7-5.7C33.6 6.1 29.1 4 24 4 12.9 4 4 12.9 4 24s8.9 20 20 20 20-8.9 20-20c0-1.3-.1-2.3-.4-3.5z" />
            <path fill="#FF3D00" d="M6.3 14.7l6.6 4.8C14.7 16 19 13 24 13c2.8 0 5.4 1.1 7.3 2.8l5.7-5.7C33.6 6.1 29.1 4 24 4 16.3 4 9.7 8.3 6.3 14.7z" />
            <path fill="#4CAF50" d="M24 44c5.2 0 9.9-2 13.4-5.2l-6.2-5.2C29.2 35 26.7 36 24 36c-5.3 0-9.7-2.6-11.3-7l-6.5 5C9.6 39.6 16.2 44 24 44z" />
            <path fill="#1976D2" d="M43.6 20.5H42V20H24v8h11.3c-.8 2.2-2.2 4.1-4.1 5.6l6.2 5.2C39.9 36.6 44 31 44 24c0-1.3-.1-2.3-.4-3.5z" />
          </svg>
          Continuar con Google
        </button>

        <div className="text-[11px] mt-6 leading-relaxed" style={{ color: "#8C95A6" }}>
          Acceso limitado a: {DOMAINS.join(" · ")}
        </div>
      </div>
    </div>
  );
}

export default function LoginPage() {
  return (
    <Suspense fallback={null}>
      <LoginInner />
    </Suspense>
  );
}
